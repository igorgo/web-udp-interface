...
const getters = {
  ...
  currentFilterEdit: state => {
    ...
    const status = state.currentFilter.claimStatus ? state.currentFilter.claimStatus.split(';') : []
    cv.claimVersion = ver
    cv.claimType = type
    cv.claimStatus = status
    cv.claimRelease = {
...
const mutations = {
  ...
  [mutateSockOk(SE_CONDITIONS_GET_ONE)] (state, {filter}) {
    state.currentFilter = {
      ...
      claimStatus: filter['P_CLAIM_STATUS'],
  ...
  [FILTER_MODIFY] (state, {key, value}) {
    switch (key) {
      ...
      case 'claimType':
        ...
        break
      case 'claimStatus':
        state.currentFilter.claimStatus = value.join(';')
        break
      default:
